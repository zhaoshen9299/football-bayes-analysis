...
淘汰赛必须同时保存90分钟比分口径和最终晋级口径。若90分钟战平而加时分出胜负，胜平负预测按“平”结算；晋级预测才按最终晋级方结算。
...
整季研究用 `scripts/backtest_league.py` 执行严格时间切分。先在验证集锁定先验权重、时间半衰期、Dixon-Coles `rho` 和市场混合权重，再对测试季逐场扩展窗口预测。每轮记录联赛主客进球、平局率和角球基线漂移；跨季明显变化时扩大先验不确定性或引入赛季层级，不能用测试季最终均值回填早期比赛。

运行前按 [backtesting.md](backtesting.md) 建立完整性台账并核验90分钟比分。L3输出本身就是市场基准；若把同一概率同时写入“模型”和“benchmark”，只能称为市场审计，不能声称独立模型击败市场。

对杯赛进行赛事内顺序更新时，基线只能由本场开球前已完成的数据构成。中立场名义主客顺序可能和种子、排名或对阵生成规则相关，不得把观察到的名义主客差直接解释为主场效应。

英超 2025/26 的一次留出验证支持 `rho=-0.10` 作为该设定的候选值，但这不是跨联赛常数；完整边界与结果见 [empirical-validation.md](empirical-validation.md)。
